...
그래서 단어 빈도만 보는 고전 모델이랑 문맥까지 보는 BERT가 성능 차이가 얼마나 나는지 비교하였다.
...
Hard dataset은 main experiment와 별도로 진행한 additional experiment이다.
더 어렵고 복잡한 텍스트 환경에서도 모델이 잘 작동하는지 검증하였으며, 특히 이 조건에서 고전 모델과 BERT의 성능 차이를 확인하였다.
