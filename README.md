...
- essay_dataset: 원본데이터 28,000건 중 5,000건 사용 ([Kaggle](https://www.kaggle.com/datasets/sunilthite/llm-detect-ai-generated-text-dataset/data))
- news_dataset: 원본데이터 58,000건 중 5,000건 사용 ([Hugging Face](https://huggingface.co/datasets/gsingh1-py/train))
- hard_dataset: 원본데이터 8,360건 중 2,247건 사용 (2,753건은 Essay, News 데이터에서 사용) ([Kaggle](https://www.kaggle.com/datasets/mahdimaktabdar/chatgpt-classification-dataset))
...
단, BERT는 max_length=128, epochs=3은 학습 시간과 자원을 고려한 설정이며, 조건을 높이면 성능이 더 향상될 것으로 예상된다.
...
4. BERT의 학습 조건 제약
BERT와 고전 모델은 동일하게 5,000개의 데이터로 학습되었으나, BERT 모델의 max_length=128, epochs=3은 학습 시간과 자원을 고려한 설정이다. 또한 Essay 99%, News 88%의 텍스트가 BERT의 max_length=128을 초과하여 잘린다. 이 두 조건이 복합적으로 작용하여 Essay에서 BERT(97.7%)가 XGBoost(97.8%)보다 오히려 낮게 나온 것으로 해석할 수 있다. BERT의 학습 조건을 더 높였다면 성능 우위가 더 뚜렷했을 가능성이 있다.
