...
| 단계 | 내용 | 적용 이유 |
|-----|------|---------|
| 1 | 깨진 문자 치환 | 같은 의미의 문자가 다르게 처리되는 것 방지 |
| 2 | 소문자 변환 | 대소문자 차이로 인한 중복 토큰 방지 |
| 3 | 특수문자 제거 | 단어 패턴 분석에 불필요한 기호 제거 |
| 4 | 알파벳만 남기기 | 숫자, 기호 등 노이즈 제거 |
| 5 | 불용어 제거 | 분류에 도움이 안 되는 공통 단어 제거 |
| 6 | Lemmatization | 같은 의미 단어를 원형으로 통일 |
| 7 | 짧은 텍스트 제거 | 학습에 충분한 정보를 가진 텍스트만 사용 |
...
즉, 전처리는 실제로 TF-IDF의 성능을 높이기 위한 필수 과정이다.
